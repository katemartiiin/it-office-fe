...
export default {
  ...
  methods: {
    async fetchItem() {
      ...
      this.$axios
        ...
        .then((response) => {
          ...
          this.payload.requestType = response.item['request']['typeofrequest']
          this.payload.completeDocs = response.item.complete_docs
          this.payload.amounts.map((amount, index) => {
            this.allotmentCodes[index] = amount.allotment_code
            this.expensesCodes[index] = amount.expenses_code
            this.amountData[index] = amount.amount
            this.requestedAmounts.push({
          ...
          this.payload.obligationNo = response.item['obligation_no']
          this.payload.requestingOfficial = response.item['requesting_official']
          this.payload.requestingOffice = response.item['requesting_office']
          this.payload.approvedAmount =
            response.item['request']['approved_amount']
          this.notes = response.notes

          if (response.item.images) {
            for (const i in response.item.images) {
              this.images.push({ path: url + '/' + response.item.images[i] })
            }
          }

          if (this.response.item.images) {
            for (const i in this.response.item.images) {
              this.images.push({
                path: url + '/' + this.response.item.images[i],
              })
            }
          }
        })
    ...
    async update() {
      ...
      this.$axios
        .$post('api/cafoa/update', this.payload, {})
        .then((res) => {
